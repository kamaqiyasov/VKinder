...
class VKSearcher:
    ...
    def search_users(self, city: str, age_from: int, age_to: int,
        ...
        # Добавляем случайный offset для разнообразия
        if offset == 0:
        ...
        for i, user in enumerate(items):
            ...
            # Пропускаем, если нельзя написать личное сообщение
            if not user.get('can_write_private_message', False):
                logger.debug(f"Пользователь {i}: нельзя написать ЛС, пропускаем")
                continue
            ...
            # Определяем пол - храним как число
            user_sex = user.get('sex')  # 1 - женский, 2 - мужской, 0 - не указан
            sex_int = user_sex
            ...
            logger.debug(f"Добавлен пользователь {i}: {user.get('first_name')} {user.get('last_name')}, пол: {sex_int}")
